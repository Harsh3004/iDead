#!/usr/bin/env python3
"""CLI entry point to execute smartphone dataset ingestion and Parquet conversion."""
...
from dataeval.ingest.smartphone import ingest_all_phone_runs
...
def main() -> int:
    ...
    default_out = repo_root / "data" / "processed" / "smartphone"

    parser = argparse.ArgumentParser(description="Ingest and deduplicate smartphone CSV dataset into Parquet.")
    parser.add_argument(
        "--raw-root",
        type=Path,
        default=default_raw,
    ...
    print(f"Starting Smartphone Ingestion:")
    print(f"  Raw root: {args.raw_root}")
    print(f"  Out dir:  {args.out_dir}")

    start_t = time.perf_counter()
    entries = ingest_all_phone_runs(args.raw_root, args.out_dir)
    ...
    variant_counts = Counter(e.schema_variant for e in ok_entries)
    rate_counts = Counter(e.sample_rate_class for e in ok_entries)
    ...
    print(f"Schema variants: {dict(variant_counts)}")
    print(f"Sample-rate classes: {dict(rate_counts)}")
